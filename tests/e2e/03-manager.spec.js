...
test.describe('Modul 3: Dasbor Manager', () => {
  ...
  test('16. Halaman Peminjaman Berlangsung (Ongoing) dapat diakses', async ({ page }) => {
    await page.goto('http://localhost:3000/manager/ongoing');
    await expect(page.locator('h1')).toContainText('Sedang Berlangsung');
  });
  test('17. Sidebar menu manajer menampilkan menu Ongoing dan Riwayat', async ({ page }) => {
  ...
  test('18. API - Badge jumlah request baru muncul di sidebar dan card', async ({ page }) => {
    await page.goto('http://localhost:3000/manager/ongoing');
  ...
  test('19. Input Select All Checkbox tersedia di tabel Ongoing', async ({ page }) => {
    await page.goto('http://localhost:3000/manager/ongoing');
    await expect(page.locator('#selectAll')).toBeVisible();
  });
  test('20. Tombol Batalkan Terpilih (Bulk Action) tersedia', async ({ page }) => {
    await page.goto('http://localhost:3000/manager/ongoing');
    await expect(page.locator('button[form="bulkCancelForm"]')).toBeVisible();
  });

  // Action Buttons (Approve/Reject/Return/Bulk)
  test('21. Aksi Terima memunculkan pop-up dialog', async ({ page }) => {
    await page.goto('http://localhost:3000/manager/ongoing');
    page.on('dialog', dialog => dialog.dismiss());
    const btn = page.locator('button:has-text("Terima")').first();
    if (await btn.isVisible()) await btn.click();
  });
  test('22. UPDATE - Menerima (Approve) mengubah status peminjaman', async ({ page }) => {
    await page.goto('http://localhost:3000/manager/ongoing');
  ...
  test('23. UPDATE - Menolak (Reject) mengubah status peminjaman', async ({ page }) => {
    await page.goto('http://localhost:3000/manager/ongoing');
    page.on('dialog', dialog => dialog.accept());
  ...
  test('24. UPDATE - Menandai dikembalikan (Return) mengubah status', async ({ page }) => {
    await page.goto('http://localhost:3000/manager/ongoing');
    page.on('dialog', dialog => dialog.accept());
  ...
  test('25. BULK UPDATE - Fitur Batalkan Terpilih berhasil memproses checklist', async ({ page }) => {
    await page.goto('http://localhost:3000/manager/ongoing');
    page.on('dialog', dialog => dialog.accept());
  ...
  test('26. Halaman Riwayat Peminjaman (History) dapat diakses', async ({ page }) => {
    await page.goto('http://localhost:3000/manager');
    await expect(page.locator('h1')).toContainText('Riwayat Peminjaman');
  });
  test('27. API - Card Total Peminjaman Selesai terisi otomatis', async ({ page }) => {
    await page.goto('http://localhost:3000/manager');
    await page.waitForTimeout(1000);
    expect(await page.locator('#stat-total').textContent()).not.toBe('-');
  });
  test('28. API - Card Total Dibatalkan terisi otomatis', async ({ page }) => {
    await page.goto('http://localhost:3000/manager');
    await page.waitForTimeout(1000);
  ...
  test('34. READ DETAIL - Halaman Detail menampilkan ID, Kode Aset, dan Tanggal', async ({ page }) => {
    await page.goto('http://localhost:3000/manager');
    const linkDetail = page.locator('a:has-text("Detail")').first();
    if (await linkDetail.isVisible()) {
      await linkDetail.click();
  ...
  test('35. Tombol Kembali di Halaman Detail berfungsi membalikkan navigasi', async ({ page }) => {
    await page.goto('http://localhost:3000/manager');
    const linkDetail = page.locator('a:has-text("Detail")').first();
    if (await linkDetail.isVisible()) {
      await linkDetail.click();
  ...
  test('36. Halaman Preview Laporan HTML dapat diakses', async ({ page }) => {
    await page.goto('http://localhost:3000/manager');
    await page.click('a:has-text("Preview Laporan")');
    await expect(page).toHaveURL(/.*preview/);
  });
  test('37. Preview Laporan menyembunyikan Sidebar & Header saat Print CSS', async ({ page }) => {
    await page.goto('http://localhost:3000/manager/report/preview');
    await expect(page.locator('.no-print').first()).toBeVisible();
  });
  test('38. EXPORT - Unduhan PDF Ongoing Report berhasil ditarik', async ({ page }) => {
    await page.goto('http://localhost:3000/manager/ongoing');
  ...
  test('39. EXPORT - Unduhan CSV History Report berhasil via URL', async ({ page }) => {
    await page.goto('http://localhost:3000/manager');
    const downloadPromise = page.waitForEvent('download');
