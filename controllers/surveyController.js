...
const exportPDF = async (req, res, next) => {
  ...
  try {
    ...
    surveys.forEach((survey, index) => {
      ...
      doc.text(
        `Tanggal Mulai : ${new Date(survey.start_date).toLocaleDateString("id-ID")}`
      );
      doc.text(
        `Tanggal Selesai : ${new Date(survey.end_date).toLocaleDateString("id-ID")}`
      );
      doc.text(
        `Status : ${survey.is_active ? "Aktif" : "Nonaktif"}`
      );
